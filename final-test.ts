/**
 * Final Coherence Test - Attack Path Algorithm
 * Tests the fixed algorithm with 1000 simulated assets
 */
...
// FIXED: Stage-based phase ordering
...
// FIXED: privilege gained based on what attacker actually gets
const privilegeGained = (vuln: Vulnerability): number => {
  if (vuln.kill_chain_phase === 'initial_access' && vuln.privileges_required === 'none') return 1
  if (vuln.kill_chain_phase === 'privilege_escalation') return 2
  if (vuln.kill_chain_phase === 'credential_access') return 2
  if (vuln.kill_chain_phase === 'lateral_movement') return 1
  return 0
}
...
const buildSparseGraphFixed = (nodes: GraphNode[]): Edge[][] => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const j of candidateIndices) {
      ...
      // FIXED: Stage-based progression - only block if jumping back more than 1 stage
...
// FIXED: Realistic probability model with detection penalty
const computeChainProbability = (path: number[], adj: Edge[][]): number => {
  if (path.length < 2) return 0.5
  
  const DETECTION_PENALTY = 0.85  // 15% detection chance per step
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    jointLogProb += Math.log(Math.max(pStep * DETECTION_PENALTY, 1e-9))
  }
  
  const pRaw = Math.exp(jointLogProb)
  return Math.min(0.99, pRaw)  // Cap at 99%, no retry inflation
}
...
const discoverAttackPathsFixed = (nodes: GraphNode[], adjList: Edge[][]): AttackPath[] => {
  ...
  // FIXED: Entry points must be internet-facing AND have initial_access
  const entryIdxs = nodes
    .map((nd, i) => ({ 
      i, 
      score: (nd.asset.internet_facing && nd.vuln.kill_chain_phase === 'initial_access') ? nd.pprScore * 1.5 : 0 
    }))
...
console.log('FINAL COHERENCE TEST - FIXED ALGORITHM')
...
const forwardPPR = computePPR(adjList, graphNodes,
  node => node.asset.internet_facing && node.vuln.kill_chain_phase === 'initial_access'
    ? safeNum(node.vuln.epss, 0.5) * (node.vuln.cisa_kev ? 2 : 1) : 0)
...
if (paths.length === 0) {
  console.log('❌ NO ATTACK PATHS FOUND - Algorithm needs more debugging!')
  process.exit(1)
}

console.log('='.repeat(80))
console.log('TOP 5 ATTACK PATHS')
console.log('='.repeat(80))
...
paths.slice(0, 5).forEach((path, idx) => {
  ...
  console.log(`│ Risk: ${path.riskScore.toFixed(1)}/10   Probability: ${(path.attackProbability * 100).toFixed(2)}%`.padEnd(79) + '│')
  ...
  path.nodes.forEach((node, stepIdx) => {
    ...
    console.log(`│    Vuln: ${node.vuln.title.substring(0, 40)}`.padEnd(79) + '│')
    console.log(`│    Phase: ${node.vuln.kill_chain_phase} | Priv: ${node.vuln.privileges_required}`.padEnd(79) + '│')
  })
  console.log(`└${'─'.repeat(78)}┘`)
})
...
console.log(`\n✓ Average path length: ${avgLength.toFixed(1)} steps`)

const allPhases = paths.flatMap(p => p.nodes.map(n => n.vuln.kill_chain_phase))
const hasInitial = allPhases.filter(p => p === 'initial_access').length
const hasCred = allPhases.filter(p => p === 'credential_access').length
const hasLateral = allPhases.filter(p => p === 'lateral_movement').length
const hasPrivEsc = allPhases.filter(p => p === 'privilege_escalation').length

console.log(`\n✓ Kill chain phases across all paths:`)
console.log(`   - initial_access: ${hasInitial} occurrences`)
console.log(`   - credential_access: ${hasCred} occurrences`)
console.log(`   - lateral_movement: ${hasLateral} occurrences`)
console.log(`   - privilege_escalation: ${hasPrivEsc} occurrences`)

const avgProb = paths.reduce((s, p) => s + p.attackProbability, 0) / paths.length
const maxProb = Math.max(...paths.map(p => p.attackProbability))
console.log(`\n✓ Probability stats:`)
console.log(`   - Average: ${(avgProb * 100).toFixed(2)}%`)
console.log(`   - Maximum: ${(maxProb * 100).toFixed(2)}%`)

if (maxProb < 0.5) {
  console.log('   ✓ Probabilities are realistic (<50% for multi-step attacks)')
} else {
  console.log('   ⚠️  Some probabilities may be too high')
}

const targets = paths.map(p => p.nodes[p.nodes.length - 1])
const highValue = targets.filter(n => n.asset.criticality >= 4).length
console.log(`\n✓ Target value: ${highValue}/${paths.length} paths end at high-criticality targets`)
...
const issues: string[] = []
if (avgLength < 3) issues.push('Short paths')
if (maxProb > 0.6) issues.push('High probabilities')

if (issues.length === 0) {
  console.log('\n✓ ALGORITHM COHERENT: Attack paths are realistic from pentester perspective')
} else {
