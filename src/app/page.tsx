...
export default function BraveGuardian() {
  ...
  const runScan = useCallback(async () => {
    ...
    setScanResults([])
    setScanJob(null)
    setView('scan') // Switch to scanner view
    setAssets(prev => prev.map(a => ({ ...a, scanStatus: 'pending' as const })))
    setStatus('Starting scan...')

    try {
      // Start scan with limited targets for performance
      const scanTargets = assets.slice(0, 100).map(a => ({
        id: a.id,
        host: a.ip,
        ip: a.ip,
        hostname: a.name,
      ...
      console.log(`[Scanner] Sending ${scanTargets.length} targets to scanner API`)
      ...
      console.log('[Scanner] API response status:', startResponse.status)

      if (startResponse.ok) {
        const data = await startResponse.json()
        const jobId = data.jobId
        console.log('[Scanner] Job started:', jobId)
        ...
        while (!completed && attempts < maxAttempts) {
          ...
          if (statusResponse.ok) {
            ...
            setScanJob(job)
            console.log(`[Scanner] Job status: ${job.status}, progress: ${job.progress}%`)
            ...
            if (job.status === 'completed' || job.status === 'failed') {
              ...
              setStatus(job.status === 'completed' ? 'Scan completed!' : 'Scan failed')
            }
          }
        }
        ...
        console.error('[Scanner] Failed to start scan:', errorText)
        setStatus(`Failed to start scan: ${startResponse.status}`)
      }
    } catch (e) {
      console.error('[Scanner] Error:', e)
      setStatus(`Scan error: ${e instanceof Error ? e.message : 'Unknown error'}`)
    }
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        {view === 'scan' && (
          <div className="space-y-6">
            ...
            {/* Start Scan Button - Show when idle or completed */}
            {(!scanJob || scanJob.status === 'completed' || scanJob.status === 'failed') && (
              <div className="bg-slate-800 rounded-xl p-8 border border-slate-700 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {scanJob?.status === 'completed' ? 'Scan Complete' : scanJob?.status === 'failed' ? 'Scan Failed' : 'Ready to Scan'}
                </h3>
                <p className="text-slate-400 mb-6">
                  {scanJob?.status === 'completed' 
                    ? `Found ${scanJob.summary?.totalMisconfigurations || 0} misconfigurations across ${scanJob.targetCount} targets`
                    : 'Click below to start scanning your network assets'}
                </p>
                <button
                  onClick={runScan}
                  disabled={scanLoading}
                  className={`px-8 py-3 rounded-lg font-medium text-base flex items-center gap-2 mx-auto ${
                    scanLoading ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'
                  }`}
                >
                  {scanLoading ? (
          ...
          <div className="space-y-6">
            {!result ? (
              <div className="text-center py-16 bg-slate-800 rounded-xl border border-slate-700">
                ...
                <p className="text-slate-400 mb-6">GNN + Bayesian + MCTS attack path analysis</p>
                <button
                  onClick={runAnalysis}
                  disabled={loading}
                  className={`px-8 py-3 rounded-lg font-medium text-base flex items-center gap-2 mx-auto ${
